(function() {
    ...
    proto.scrape = function(files, delimiters) {
        var data = scraper.scrapeFiles(files, delimiters);
    ...
    proto.load = function(components) {
        if (!fileExists(components)) {
            return;
        }

        var data = JSON.parse(readFile(components));
    ...
    proto.map = function(data) {
        ...
        var componentModel = new ComponentModel().fromJSON({
            ...
            description: fileExists(file) ? marked(swig.render(readFile(file))) : marked(description),
