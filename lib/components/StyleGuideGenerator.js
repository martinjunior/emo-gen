(function() {
    ...
    var StyleGuideGenerator = function(options, nunjucksOptions) {
        ...
        // normalize the src/dest file paths
        this.options.path.src = path.normalize(this.options.path.src);
        this.options.path.dest = path.normalize(this.options.path.dest);
    ...
    StyleGuideGenerator.OPTIONS = {
        path: {
            src: path.normalize('styleguide/src/'),
            dest: path.normalize('styleguide/dest/')
        },
        searchPaths: [],
        DEFAULT_COMPONENT_TEMPLATE: path.normalize('templates/component.html')
    };
