(function() {
    ...
    StyleGuideGenerator.OPTIONS = {
        path: {
            _src: path.normalize('styleguide/src/'),
            set src(value) {
                this._src = path.normalize(path.join(value, path.sep));

                console.log(value, this._src);
            },
            get src() {
                console.log('Getting src: ' + this._src);

                return this._src;
            },
            _dest: path.normalize('styleguide/dest/'),
            set dest(value) {
                this._dest = path.normalize(path.join(value, path.sep));

                console.log(value, this._dest);
            },
            get dest() {
                console.log('Getting dest: ' + this._dest);
